      ...
      <section v-if="currentTicker" class="relative">
        ...
        <div class="flex items-end border-gray-600 border-b border-l h-64">
          <div
            ...
            v-for="(el, index) in normalizeGraph()"
            :key="index"
          ></div>
        </div>
        <button
...
export default {
  ...
  methods: {
    addTicker() {
      ...
      setInterval(async () => {
        const f = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${newTicker.name}&tsyms=USD&api_key=bbe1a6e062415b6aa51b27f31a3ca676cc612f89cfcdad86be9a74d644c9bc1c`
        );
    ...
    normalizeGraph() {
      ...
      return this.graph.map(
        (el) => 5 + ((el - minValue) * 95) / (maxValue - minValue)
      );
    },
    selectTicker(ticker) {
